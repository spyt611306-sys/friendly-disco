...
class BaseCollector:
    ...
    def evaluate_relevance(self, text: str) -> Dict[str, Any]:
        ...
        if is_cctv_noise(source) or (any(term in source for term in NOISE_TERMS) and not any(term in source for term in MARINE_KEYWORDS)):
            return {
        ...
        platform = [term for term in MARINE_KEYWORDS if term in source]
        watchlist = [term for term in [*WATCHLIST_TERMS, *env_terms("SHIP_WATCHLIST_TERMS")] if term.lower() in source]
        shipyards = [term for term in [*SHIPYARD_TERMS, *env_terms("SHIPYARD_WATCHLIST")] if term.lower() in source]
        builds = [term for term in [*BUILD_TERMS, *env_terms("VESSEL_BUILD_WATCHLIST")] if term.lower() in source]
        buyers = [term for term in BUYER_WEIGHTS if term.lower() in source]
        equipment = [term for term in EQUIPMENT_WEIGHTS if term.lower() in source]
        has_electric = any(term in source for term in ["전기추진", "하이브리드", "hybrid", "battery", "배터리", "ess", "dc grid"])
        has_drive = any(term in source for term in ["인버터", "inverter", "드라이브", "drive", "vfd", "afe", "waterjet", "워터젯"])
        has_motor = any(term in source for term in ["추진", "모터", "motor", "축발전기", "shaft generator"])
        has_power = any(term in source for term in ["배터리", "battery", "ess", "dc grid", "배전", "switchboard", "pcs"])

        relevance = min(38, len(platform) * 6 + len(builds) * 4 + len(watchlist) * 4 + (8 if buyers else 0))
        solution_fit = min(35, sum(EQUIPMENT_WEIGHTS[term] for term in equipment))
        buyer_fit = min(18, sum(BUYER_WEIGHTS[term] for term in buyers))
        score = min(100, relevance + solution_fit + buyer_fit + (7 if shipyards and builds else 0))
        if not platform and not builds and not watchlist and not shipyards:
            score = 0
        is_marine_candidate = bool(platform or builds or watchlist or shipyards)
    ...
    def build_project(self, operation_name: str, item: Dict[str, Any], source_url: str) -> Optional[Dict[str, Any]]:
        ...
        source_id = hashlib.sha1(f"{source_url}|{title}".encode("utf-8")).hexdigest()[:16]
