...
class PublicDataCollector(BaseCollector):
    ...
    async def collect(self, seed_projects: List[Dict[str, str]] | None = None) -> List[Dict[str, str]]:
        ...
        bid_start = today - timedelta(days=get_env_int("PUBLIC_DATA_LOOKBACK_DAYS", 30) - 1)
        bid_params = {
        ...
        contract_start = today - timedelta(days=get_env_int("PUBLIC_DATA_LOOKBACK_DAYS", 30) - 1)
        contract_params = {
        ...
        award_start = today - timedelta(days=get_env_int("PUBLIC_DATA_LOOKBACK_DAYS", 30) - 1)
        award_windows = chunk_days(award_start, today, get_env_int("G2B_WINDOW_DAYS", 30))
