...
VERIFY_AFTER_COLLECT = os.getenv("VERIFY_AFTER_COLLECT", "true").strip().lower() in {"1", "true", "yes", "on"}
VERIFY_MAX_PROJECTS = max(1, min(200, int(os.getenv("VERIFY_MAX_PROJECTS", "30"))))
DEFAULT_PIPELINE_ALIASES = [
    item.strip()
    for item in os.getenv(
        "DEFAULT_PIPELINE_ALIASES",
        "order_plan,procurement_request,prespec,bid,award,contract,public_data,market_signal,news,public_notice,ship_support,integrated_process",
    ).split(",")
    if item.strip()
]

STAGES = ["LEAD", "PLAN", "REQUEST", "PRESPEC", "BID", "EVALUATION", "CONTRACT", "BUILD", "DELIVERED"]
...
def make_registry() -> Dict[str, Any]:
    return {
        ...
        "integrated_process": IntegratedProcessCollector(),
    }
...
def identity_key(project: Dict[str, Any]) -> str:
    ...
    for term in ("입찰공고", "사전규격", "낙찰결과", "계약현황", "발주계획", "조달요청"):
...
def merge_raw_payloads(current: Dict[str, Any], incoming: Dict[str, Any]) -> Dict[str, Any]:
    merged = {**(current or {}), **(incoming or {})}
    current_target = (current or {}).get("_abbTargeting") or {}
    incoming_target = (incoming or {}).get("_abbTargeting") or {}
    merged["_abbTargeting"] = (
        incoming_target
        if int(incoming_target.get("score") or 0) >= int(current_target.get("score") or 0)
        else current_target
    )
    current_verification = (current or {}).get("_verification") or {}
    incoming_verification = (incoming or {}).get("_verification") or {}
    if current_verification or incoming_verification:
        merged["_verification"] = (
            incoming_verification
            if int(incoming_verification.get("confidence") or 0) >= int(current_verification.get("confidence") or 0)
            else current_verification
        )
    current_sales = (current or {}).get("_salesIntelligence") or {}
    incoming_sales = (incoming or {}).get("_salesIntelligence") or {}
    if current_sales or incoming_sales:
        class_rank = {"P0": 0, "P1": 1, "P2": 2, "P3": 3}
        merged_sales = {**current_sales, **incoming_sales}
        merged_sales["opportunityClass"] = min(
            [str(current_sales.get("opportunityClass") or "P3"), str(incoming_sales.get("opportunityClass") or "P3")],
            key=lambda value: class_rank.get(value, 3),
        )
        for field in ("solutionAreas", "salesReasons", "riskFlags"):
            merged_sales[field] = list(dict.fromkeys((current_sales.get(field) or []) + (incoming_sales.get(field) or [])))
        merged["_salesIntelligence"] = merged_sales
    document_groups = []
    for payload in (current or {}, incoming or {}):
        document_groups.extend(((payload.get("_documents") or {}).get("documents") or []))
    if document_groups:
        documents = merge_unique(document_groups, ("url", "sourceId"))
        merged["_documents"] = {
            "analyzedCount": sum(1 for item in documents if item.get("status") == "ANALYZED"),
            "documents": documents,
        }
    return merged
...
def merge_projects(current: Dict[str, Any], incoming: Dict[str, Any]) -> Dict[str, Any]:
    ...
    merged["rawPayload"] = merge_raw_payloads(merged.get("rawPayload") or {}, incoming.get("rawPayload") or {})
    if int(incoming.get("verificationConfidence") or 0) >= int(merged.get("verificationConfidence") or 0):
        for field in ("verificationStatus", "verificationConfidence", "verificationCheckedAt"):
...
UPSERT_SQL = """
    INSERT INTO projects (
        ...
        source_type, source_service, source_operation, verification_status, verification_confidence,
        verification_checked_at, matched_keywords,
        ...
        %(source_service)s, %(source_operation)s, %(verification_status)s, %(verification_confidence)s,
        %(verification_checked_at)s, %(matched_keywords)s,
    ...
    ON CONFLICT (dedupe_key) DO UPDATE SET
        ...
        stage = CASE
            WHEN array_position(ARRAY['LEAD','PLAN','REQUEST','PRESPEC','BID','EVALUATION','CONTRACT','BUILD','DELIVERED'], EXCLUDED.stage)
               >= array_position(ARRAY['LEAD','PLAN','REQUEST','PRESPEC','BID','EVALUATION','CONTRACT','BUILD','DELIVERED'], projects.stage)
        ...
        verification_status = CASE
            WHEN EXCLUDED.verification_confidence >= projects.verification_confidence
            THEN EXCLUDED.verification_status ELSE projects.verification_status END,
        verification_confidence = GREATEST(projects.verification_confidence, EXCLUDED.verification_confidence),
        verification_checked_at = CASE
            WHEN EXCLUDED.verification_confidence >= projects.verification_confidence
            THEN COALESCE(EXCLUDED.verification_checked_at, projects.verification_checked_at)
            ELSE projects.verification_checked_at END,
        matched_keywords = (
...
def save_projects(projects: List[Dict[str, Any]]) -> int:
    ...
    with closing(get_db_connection()) as conn:
        with conn.cursor() as cur:
            cur.execute(
                "SELECT dedupe_key, identity_key, announcement_no, contract_no, project_no, stage, "
                "raw_payload, matched_keywords, history, sources FROM projects"
            )
            existing = cur.fetchall()
            key_map: Dict[str, str] = {}
            existing_by_key: Dict[str, Dict[str, Any]] = {row["dedupe_key"]: row for row in existing}
            for row in existing:
            ...
            for project in projects:
                ...
                current = existing_by_key.get(resolved)
                if current:
                    project["rawPayload"] = merge_raw_payloads(current.get("raw_payload") or {}, project.get("rawPayload") or {})
                    project["matchedKeywords"] = sorted(set((current.get("matched_keywords") or []) + (project.get("matchedKeywords") or [])))
                    project["keywordText"] = " ".join(project["matchedKeywords"])
                    project["history"] = merge_unique((current.get("history") or []) + (project.get("history") or []), ("id", "date", "action"))
                    project["sources"] = merge_unique((current.get("sources") or []) + (project.get("sources") or []), ("url", "title"))
                    old_stage = str(current.get("stage") or "LEAD").upper()
                    new_stage = str(project.get("stage") or "LEAD").upper()
                    project["stage"] = new_stage if STAGE_RANK.get(new_stage, 0) >= STAGE_RANK.get(old_stage, 0) else old_stage
                verification = (project.get("rawPayload") or {}).get("_verification") or {}
                params = {
                    "dedupe_key": resolved,
                    ...
                    "verification_confidence": int(project.get("verificationConfidence") or verification.get("confidence") or 0),
                    "verification_checked_at": project.get("verificationCheckedAt") or verification.get("checkedAt"),
                ...
                existing_by_key[resolved] = {
                    "dedupe_key": resolved,
                    "stage": params["stage"],
                    "raw_payload": project.get("rawPayload") or {},
                    "matched_keywords": project.get("matchedKeywords") or [],
...
def extract_sales_intelligence(raw_payload: Dict[str, Any]) -> Dict[str, Any]:
    sales = (raw_payload or {}).get("_salesIntelligence") or {}
    opportunity_class = str(sales.get("opportunityClass") or "P2").upper()
    if opportunity_class not in {"P0", "P1", "P2", "P3"}:
        opportunity_class = "P2"
    return {
        "opportunityClass": opportunity_class,
        "solutionAreas": sales.get("solutionAreas") or [],
        "salesReasons": sales.get("salesReasons") or [],
        "riskFlags": sales.get("riskFlags") or [],
        "recommendedAction": sales.get("recommendedAction") or "공식 근거와 다음 발주 단계를 확인하세요.",
    }


def evidence_snapshot(raw_payload: Dict[str, Any], sources: List[Dict[str, Any]]) -> Dict[str, Any]:
    evidence = (raw_payload or {}).get("_evidence") or {}
    documents = (raw_payload or {}).get("_documents") or {}
    official = [
        source for source in sources
        if str(source.get("evidenceKind") or "").upper().startswith("OFFICIAL")
        or str(source.get("type") or "").upper() in {"G2B", "PUBLIC_STD", "PUBLIC_NOTICE", "SHIP", "G2B_DOCUMENT"}
    ]
    direct = [
        source for source in official
        if source.get("isDirectLink") is not False and str(source.get("url") or "").startswith(("https://", "http://"))
    ]
    attachments = [source for source in official if str(source.get("evidenceKind") or "").upper() == "OFFICIAL_ATTACHMENT"]
    return {
        "officialCount": len(official),
        "directOfficialCount": len(direct),
        "apiOnlyCount": max(0, len(official) - len(direct)),
        "attachmentCount": len(attachments),
        "analyzedDocumentCount": int(documents.get("analyzedCount") or 0),
        "identifiers": evidence.get("identifiers") or {},
        "searchHint": evidence.get("searchHint") or "",
    }
...
def row_to_project(row: Dict[str, Any]) -> Dict[str, Any]:
    ...
    abb = extract_abb(raw)
    sales = extract_sales_intelligence(raw)
    verification = raw.get("_verification") or {}
    sources = row.get("sources") or []
    evidence = evidence_snapshot(raw, sources)
    ...
    return {
        ...
        "verificationConfidence": int(row.get("verification_confidence") or verification.get("confidence") or 0),
        "verificationCheckedAt": (
            row["verification_checked_at"].isoformat()
            if hasattr(row.get("verification_checked_at"), "isoformat")
            else row.get("verification_checked_at") or verification.get("checkedAt")
        ),
        "verificationReason": verification.get("reason") or "",
        "verificationProvider": verification.get("provider") or "",
        "verificationWarnings": verification.get("warnings") or [],
        "officialEvidenceCount": int(verification.get("officialEvidenceCount") or 0),
        "newsEvidenceCount": int(verification.get("newsEvidenceCount") or 0),
...
def project_data_quality(projects: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    return {
        ...
        "verifiedCount": sum(
            ...
            if item.get("verificationStatus") in {"CROSS_VERIFIED", "OFFICIAL_CONFIRMED", "NEWS_CORROBORATED"}
        ),
        "crossVerifiedCount": sum(1 for item in projects if item.get("verificationStatus") == "CROSS_VERIFIED"),
...
def fetch_project_by_id(project_id: str) -> Dict[str, Any]:
    with closing(get_db_connection()) as conn:
        with conn.cursor() as cur:
            cur.execute("SELECT * FROM projects WHERE id = %s OR dedupe_key = %s LIMIT 1", (project_id, project_id))
            row = cur.fetchone()
            if not row:
...
def save_followup(project_id: str, values: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if "stage" in values:
        ...
        params.append(stage or "LEAD")
    if "nextActionDate" in values:
        action_date = values.get("nextActionDate") or None
    else:
        action_date = None
    if "nextActionDate" in values and action_date:
        try:
            date.fromisoformat(action_date)
        except ValueError as exc:
            raise ValueError("nextActionDate는 YYYY-MM-DD 형식이어야 합니다.") from exc
    field_map = {
        "owner": "owner", "nextActionDate": "next_action_date", "nextAction": "next_action",
        "notes": "notes", "favorite": "favorite",
    }
    for api_field, db_column in field_map.items():
        if api_field not in values:
            continue
        value = values.get(api_field)
        if api_field != "favorite":
            value = value or None
    ...
    with closing(get_db_connection()) as conn:
        with conn.cursor() as cur:
            assignments.extend(["history = history || %s::jsonb", "updated_at = NOW()"])
            params.extend([json.dumps([event], ensure_ascii=False), project_id, project_id])
            cur.execute(
                f"UPDATE projects SET {', '.join(assignments)} WHERE id = %s OR dedupe_key = %s RETURNING *",
                params,
            )
            row = cur.fetchone()
            if not row:
...
async def verify_recent_projects() -> int:
    projects = await asyncio.to_thread(fetch_seed_projects)
    candidates = sorted([
        project for project in projects
        if project.get("verificationStatus") != "CROSS_VERIFIED"
    ], key=lambda project: (
        {"P0": 3, "P1": 2, "P2": 1, "P3": 0}.get(str(project.get("opportunityClass") or "P3"), 0),
        int(project.get("abbScore") or 0),
        len(project.get("sources") or []),
    ), reverse=True)[:VERIFY_MAX_PROJECTS]
    if not candidates:
        return 0
    verifier = ProjectVerifier()
    try:
        verified = await verifier.verify_many(candidates, limit=VERIFY_MAX_PROJECTS)
        await asyncio.to_thread(save_projects, verified)
        return len(verified)
    finally:
    ...
    try:
        ...
        for index, alias in enumerate(aliases):
            ...
            try:
                ...
                # The process-linking collector must see identifiers gathered earlier in this run.
                if saved:
                    seed = await asyncio.to_thread(fetch_seed_projects)
        ...
        if VERIFY_AFTER_COLLECT and saved_total > 0:
            COLLECT_JOB.update(
                currentStep="verification",
                progress=92,
                message="공식 데이터와 뉴스 근거를 교차 검증하고 있습니다.",
            )
            try:
                verified_count = await verify_recent_projects()
                COLLECT_JOB["verifiedCount"] = verified_count
            except Exception as exc:
                logger.error("프로젝트 교차 검증 실패: %s", exc)
                errors.append({"collector": "verification", "message": str(exc)[:500]})
                COLLECT_JOB["errors"] = errors
...
def health() -> JSONResponse:
    try:
        ...
        logger.warning("헬스체크 DB 연결 실패: %s", exc)
        return JSONResponse(
            {"status": "degraded", "database": "unavailable", "version": app.version},
            status_code=503,
        )
...
@app.post("/api/projects/{project_id}/verify", dependencies=[Depends(require_admin)])
async def verify_project(project_id: str) -> Dict[str, Any]:
    try:
        project = await asyncio.to_thread(fetch_project_by_id, project_id)
        verifier = ProjectVerifier()
        try:
            verified = await verifier.verify(project, search_external=True)
        finally:
            await verifier.aclose()
        await asyncio.to_thread(save_projects, [verified])
        refreshed = await asyncio.to_thread(fetch_project_by_id, project_id)
        return {"project": refreshed, "message": refreshed.get("verificationReason") or "근거 재검증이 완료되었습니다."}
    except KeyError as exc:
        raise HTTPException(status_code=404, detail="프로젝트를 찾을 수 없습니다.") from exc
    except Exception as exc:
        logger.exception("프로젝트 검증 실패")
        raise HTTPException(status_code=502, detail=f"프로젝트 근거 검증 실패: {exc}") from exc
...
def api_meta() -> Dict[str, Any]:
    ...
    return {
        ...
        "verification": {
            "automatic": VERIFY_AFTER_COLLECT,
            "naverNewsConfigured": bool(os.getenv("NAVER_CLIENT_ID") and os.getenv("NAVER_CLIENT_SECRET")),
            "googleNewsRssEnabled": os.getenv("GOOGLE_NEWS_VERIFY_ENABLED", "true").lower() in {"1", "true", "yes", "on"},
        },
        "documents": {
            "analysisEnabled": os.getenv("DOCUMENT_ANALYSIS_ENABLED", "true").lower() in {"1", "true", "yes", "on"},
            "maxProjectsPerRun": int(os.getenv("DOCUMENT_MAX_PROJECTS_PER_RUN", "8")),
        },
    }
