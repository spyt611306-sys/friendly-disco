...
from sales_intelligence import CLASSIFICATION_VERSION, classify_sales_opportunity, project_sales_text
...
DEFAULT_PIPELINE_ALIASES = [
    ...
    for item in os.getenv(
        "DEFAULT_PIPELINE_ALIASES",
        "bid,contract,award,prespec,order_plan,public_data,market_signal,news,public_notice,ship_support",
    ).split(",")
    if item.strip()
]
...
async def lifespan(_: FastAPI):
    try:
        ...
        try:
            reclassified = await asyncio.to_thread(reclassify_existing_projects)
            if reclassified:
                logger.info("기존 프로젝트 영업 적합도 재분류 완료: %s건", reclassified)
        except Exception as exc:
            logger.warning("기존 프로젝트 영업 적합도 재분류 보류: %s", exc)
    except Exception as exc:
...
def ensure_tables() -> None:
    with closing(get_db_connection()) as conn:
        with conn.cursor() as cur:
            ...
            cur.execute("CREATE INDEX IF NOT EXISTS idx_projects_sales_focus ON projects(sales_category, sales_score DESC)")
            conn.commit()
...
def make_registry() -> Dict[str, Any]:
    return {
        ...
        "market_signal": MarketSignalCollector(),
        "news": NewsCollector(),
...
UPSERT_SQL = """
    INSERT INTO projects (
        ...
        verification_checked_at, sales_category, sales_score, classification_version, matched_keywords,
        ...
        %(verification_checked_at)s, %(sales_category)s, %(sales_score)s, %(classification_version)s,
        %(matched_keywords)s,
    ...
    ON CONFLICT (dedupe_key) DO UPDATE SET
        ...
        sales_category = CASE
            WHEN EXCLUDED.classification_version >= projects.classification_version
            THEN EXCLUDED.sales_category ELSE projects.sales_category END,
        sales_score = CASE
            WHEN EXCLUDED.classification_version >= projects.classification_version
            THEN EXCLUDED.sales_score ELSE projects.sales_score END,
        classification_version = GREATEST(projects.classification_version, EXCLUDED.classification_version),
        matched_keywords = (
    ...
    with closing(get_db_connection()) as conn:
        with conn.cursor() as cur:
            ...
            for project in projects:
                ...
                targeting = extract_abb(project.get("rawPayload") or {}, project)
                params = {
                    "dedupe_key": resolved,
                    ...
                    "sales_category": targeting["salesCategory"],
                    "sales_score": targeting["score"],
...
def extract_abb(raw_payload: Dict[str, Any], project: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    meta = (raw_payload or {}).get("_abbTargeting") or {}
    if int(meta.get("classificationVersion") or 0) < CLASSIFICATION_VERSION and project:
        result = classify_sales_opportunity(project_sales_text({**project, "rawPayload": raw_payload}))
        meta = {
            "classificationVersion": result["classification_version"],
            "score": result["score"],
            "priority": result["priority"],
            "salesCategory": result["sales_category"],
            "opportunityType": result["opportunity_type"],
            "salesTiming": result["sales_timing"],
            "confidence": result["confidence"],
            "reason": result["reason"],
            "recommendedAction": result["recommended_action"],
            "detail": {
                "buyerKeywords": result["buyer_keywords"],
                "equipmentKeywords": result["equipment_keywords"],
                "platformKeywords": result["platform_keywords"],
                "shipyardKeywords": result["shipyard_keywords"],
                "buildKeywords": result["build_keywords"],
                "watchlistHits": result["watchlist_hits"],
                "earlySignalKeywords": result["early_signal_keywords"],
                "excludeKeywords": result["exclude_keywords"],
                "componentScores": {
                    "driveScore": result["drive_score"],
                    "motorScore": result["motor_score"],
                    "powerScore": result["power_score"],
                },
            },
        }
    ...
    return {
        ...
        "priority": meta.get("priority") or "REFERENCE",
        "salesCategory": meta.get("salesCategory") or "REFERENCE",
        "opportunityType": meta.get("opportunityType") or "MARINE_MONITOR",
        "salesTiming": meta.get("salesTiming") or "MONITOR",
        "classificationConfidence": int(meta.get("confidence") or 0),
        "salesReason": meta.get("reason") or "",
        "recommendedAction": meta.get("recommendedAction") or "",
        ...
        "earlySignalKeywords": detail.get("earlySignalKeywords") or [],
        "excludeKeywords": detail.get("excludeKeywords") or [],
    }


def targeting_meta_from_result(result: Dict[str, Any]) -> Dict[str, Any]:
    return {
        "classificationVersion": result["classification_version"],
        "score": result["score"],
        "priority": result["priority"],
        ...
        "recommendedAction": result["recommended_action"],
        "detail": {
            "gateKeywords": result["platform_keywords"],
            "buyerKeywords": result["buyer_keywords"],
            "equipmentKeywords": result["equipment_keywords"],
            "platformKeywords": result["platform_keywords"],
            "shipyardKeywords": result["shipyard_keywords"],
...
def reclassify_existing_projects() -> int:
    """분류기 버전이 오래된 DB 행을 원본 보존 상태로 일괄 재평가한다."""
    ...
    with closing(get_db_connection()) as conn:
        with conn.cursor() as cur:
            cur.execute(
                """
                SELECT dedupe_key, name, company, shipyard, keyword_text, raw_payload
                FROM projects
            ...
            for row in rows:
                ...
                result = classify_sales_opportunity(project_sales_text(project))
                raw = {**(row.get("raw_payload") or {}), "_abbTargeting": targeting_meta_from_result(result)}
                cur.execute(
                    """
                    UPDATE projects
                    SET sales_category = %s, sales_score = %s, classification_version = %s, raw_payload = %s
                    WHERE dedupe_key = %s
                    """,
                    (
                        result["sales_category"], result["score"], CLASSIFICATION_VERSION,
                        Json(raw), row["dedupe_key"],
                    ),
                )
                updated_count += 1
            conn.commit()
...
def row_to_project(row: Dict[str, Any]) -> Dict[str, Any]:
    ...
    abb = extract_abb(raw, classification_project)
    ...
    return {
        ...
        "salesCategory": abb["salesCategory"],
        "opportunityType": abb["opportunityType"],
        "salesTiming": abb["salesTiming"],
        "classificationConfidence": abb["classificationConfidence"],
        "salesReason": abb["salesReason"],
        "recommendedAction": abb["recommendedAction"],
        ...
        "earlySignalKeywords": abb["earlySignalKeywords"],
        "excludeKeywords": abb["excludeKeywords"],
    }
        ...
        with conn.cursor() as cur:
            ...
            cur.execute(
                ...
                ORDER BY favorite DESC,
                    CASE sales_category
                        WHEN 'DIRECT_SALES' THEN 4 WHEN 'EARLY_PROJECT' THEN 3
                        WHEN 'WATCH' THEN 2 WHEN 'REFERENCE' THEN 1 ELSE 0 END DESC,
                    sales_score DESC, updated_at DESC
                LIMIT %s OFFSET %s""",
                [*params, limit, offset],
            )
    ...
    with closing(get_db_connection()) as conn:
        with conn.cursor() as cur:
            cur.execute(
                ...
                ORDER BY CASE sales_category
                    WHEN 'DIRECT_SALES' THEN 4 WHEN 'EARLY_PROJECT' THEN 3
                    WHEN 'WATCH' THEN 2 WHEN 'REFERENCE' THEN 1 ELSE 0 END DESC,
                    sales_score DESC, updated_at DESC LIMIT 300"""
            )
...
async def verify_recent_projects() -> int:
    ...
    candidates = sorted([
        ...
        and project.get("salesCategory") in {"DIRECT_SALES", "EARLY_PROJECT", "WATCH"}
    ], key=lambda item: (
        {"DIRECT_SALES": 3, "EARLY_PROJECT": 2, "WATCH": 1}.get(item.get("salesCategory"), 0),
        int(item.get("abbScore") or 0),
        ...
        "collectors": [
            {
                ...
                "configured": collector.configured() if hasattr(collector, "configured") else (
                    bool(collector.api_key()) if collector.operations else bool(
